...
const app = express();
Cfonts.say("ALUXI - MD", { font: "tiny" });
app.get("/", (req, res) => {
  res.setHeader("Content-Type", "application/json");
  const data = {
    status: "true",
    message: "aluxi is now running",
    author: "alvianuxio",
  };
...
function start(file) {
  ...
  p.on("exit", (code) => {
    isRunning = false;
    console.error(`bot mati dengan kode: ${code}`);
  ...
  fs.readdir(pluginsFolder, (err, files) => {
    ...
    let table = `${chalk.blue.bold("© ALUXI - MD")}
${chalk.yellow.bold("××××××××××××××××××××××")}`;
    console.log(table);
  });
